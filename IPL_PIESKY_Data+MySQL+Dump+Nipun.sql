...
-- Question1.	Show the percentage of wins of each bidder in the order of highest to lowest percentage.
...
-- Question2.	Display the number of matches conducted at each stadium with stadium name,city from the database.
...
-- Question3.	In a given stadium, what is the percentage of wins by a team which has won the toss?
select stadium_id ,stadium_name ,(select count(*) from ipl_match im join ipl_match_schedule ims on im.match_id=ims.match_id 
where ims.stadium_id=ist.stadium_id and (toss_winner=match_winner))/
(select count(*) from ipl_match_schedule ims where ims.stadium_id = ist.stadium_id) * 100 
as 'Percentage of Wins by teams who won the toss (%)'
from ipl_stadium ist;

-- Question4.	Show the total bids along with bid team and team name.
select * from ipl_team;
select * from ipl_bidding_details;
select team_id,team_name,count(*) from ipl_team it
join ipl_bidding_details bdt on it.team_id=bdt.bid_team
group by team_id;

-- Question5.	Show the team id who won the match as per the win details.
...
-- Question6.	Display total matches played, total matches won and total matches lost by team along with its team name.
select it.team_id, team_name,matches_played,matches_won,matches_lost from ipl_team it 
join ipl_team_standings its
 on it.team_id=its.team_id 
 group by team_id ;

-- Question7.	Display the bowlers for Mumbai Indians team.
select * from ipl_team_players;
select it.team_id,team_name,player_name,itp.player_id,player_role from ipl_team it 
...
-- Question8.	How many all-rounders are there in each team, Display the teams with more than 4 all-rounder in descending order
select it.team_id,team_name,player_role,count(player_role) from ipl_team it
join ipl_team_players itp on it.team_id=itp.team_id
